...
const Icons = {
  ...
  Edit: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>, // Novo ícone
};
      ...
      <aside className="w-64 border-r border-slate-800/60 bg-[#020617] flex flex-col hidden md:flex z-20 relative">
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-8">
          ...
          <div>
            ...
            <nav className="space-y-1">
              {allTags.map(tag => (
                <button key={tag} onClick={() => setActiveTag(tag)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${activeTag === tag ? 'bg-cyan-900/20 text-cyan-400' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'}`}>
                  {Icons.Hash} {tag}
                </button>
              ))}
            </nav>
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-20 relative z-10 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8 items-center justify-between">
            <div className="relative w-full md:w-96 group">
              ...
              <input type="text" placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-[#0B1121] border border-slate-800 rounded-lg py-2.5 pl-10 pr-4 text-sm text-slate-200 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all placeholder:text-slate-600" />
            </div>
              ...
              <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                <div className="flex justify-between items-center mb-2">
                   <h3 className="text-sm font-bold text-white">
                      {editingId ? "Editar Registro" : "Registrar Novo Erro"}
                   </h3>
                </div>
                ...
                <div className="flex justify-end pt-2 gap-3">
                  ...
                  <button type="submit" className="bg-cyan-600 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:bg-cyan-500">
                ...
                <div className="p-5 flex flex-col h-full pointer-events-none">
                  <div className="flex justify-between items-start mb-3 relative z-10 pointer-events-auto">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {/* BOTÃO EDITAR */}
                      <button 
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); startEditing(note); }} 
                        className="text-slate-600 hover:text-cyan-400 p-1" title="Editar"
                      >
                        {Icons.Edit}
                      </button>
                      {/* BOTÃO DELETAR */}
                      <button 
                        ...
                        className="text-slate-600 hover:text-red-400 p-1" title="Excluir"
                      >
                        {Icons.Trash}
                      </button>
                    </div>
